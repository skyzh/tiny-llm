...
Dense reconstruction, paged storage plus compatibility gather, and direct
paged attention run in fresh alternating processes. The warmup compiles a
complete workload. The benchmark then synchronizes and resets every page pool,
so each measured paged process starts with zero page capacity. No global
maximum context length is passed to the cache. Each request still has a
generated-token limit so the finite benchmark can complete.

The runner reports:

| Metric | What it reveals |
|---|---|
| Prefill tok/s | chunked admission and long-query attention work |
...
Direct paging improves output and request throughput by 37.4%, aggregate decode
by 85.9%, and lowers peak KV storage by 47.4% relative to dense reconstruction.
Avoidable logical copy volume falls 99.8%. Prefill is 0.5% lower at the
128-token chunk size, so the serving result does not claim a short-chunk
FlashAttention win. The direct run reaches 1,116 live pages out of 1,152
reserved, reuses 2,196 allocations, and records 15,840 unused tail slots across
the layer caches. Those are the claims this workload supports.
