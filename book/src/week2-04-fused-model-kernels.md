...
those interfaces and make the CPU path report that this course extension
needs a GPU.
...
`FastRoPE.__call__` in `src/tiny_llm/week2_kernels.py`. Keep the CPU path's
explicit GPU-only error.
...
`src/tiny_llm/week2_kernels.py`. Preserve the CPU path's GPU-only error.
