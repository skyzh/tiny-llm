...
In week 1, you will implement the necessary components in Python (only Python!) to use the Qwen3 model to generate responses (e.g., attention, RoPE, etc). In week 2, **Forging the Fast Path**, you will optimize the single-request decode path with efficient Metal kernels and work toward 80-90% of MLX's performance. In week 3, **Build a Mini vLLM**, you will turn that fast model into a serving engine with continuous batching, optimized prefill, and paged attention. In week 4, you will use the engine to build applications such as coding agents and RAG.
