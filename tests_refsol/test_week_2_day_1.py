...
def helper_test_task_3(model_name: str, iters: int = 10):
    ...
    for _ in range(iters):
        ...
        assert_allclose(
            user_output, ref_output, precision=mx.bfloat16, rtol=0.1, atol=0.5
        )
...
def helper_test_task_4(model_name: str, seq_len: int, iters: int = 1):
    ...
    for _ in range(iters):
        ...
        for offset in range(seq_len):
            ...
            assert_allclose(user_out, ref_out, precision=mx.bfloat16, rtol=1e-1)
...
def test_task_4_qwen_3_4b():
    helper_test_task_4("Qwen/Qwen3-4B-MLX-4bit", seq_len=3)
