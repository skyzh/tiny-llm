...
def test_task_3_qwen2_grouped_query_attention(stream: mx.Stream, precision: mx.Dtype, mask: str | None):
    with mx.stream(stream): 
        batch_size = 1
        seq_len = 4
        hidden_size = 32
        ...
        mx.random.seed(42)
        x = mx.random.uniform(-1.0, 1.0, shape=(batch_size, seq_len, hidden_size), dtype=precision)
