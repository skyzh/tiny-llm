...
WEEK2_VARIANTS = (
    ...
    Variant(
        ...
        "2.6 Shared-input QKV",
        "ref",
        "week2",
        ("--week2-checkpoint", "shared-input-qkv"),
    ),
    Variant(
        "week2-shared-input-gate-up-swiglu",
        "2.6 Shared-input gate+up/SwiGLU",
        "ref",
        "week2",
        ("--week2-checkpoint", "shared-input-gate-up-swiglu"),
    ),
    Variant(
        "week2-io-aware-dense-attention",
        "2.7 I/O-aware dense attention",
        "ref",
        "week2",
        ("--week2-checkpoint", "io-aware-dense-attention"),
    ),
    MLX_VARIANT,
)
VARIANTS_BY_KEY = {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--disable-week2-shared-input-qkv",
        action="store_true",
        help="disable only the Week 2 shared-input QKV checkpoint",
    )
    parser.add_argument(
        "--disable-week2-io-aware-dense-attention",
        action="store_true",
        help="disable only the Week 2 I/O-aware dense-attention checkpoint",
    )
    parser.add_argument(
        "--disable-week2-shared-input-gate-up-swiglu",
        action="store_true",
        help="disable only the Week 2 shared-input gate+up/SwiGLU checkpoint",
    )
...
def run_variant(
    ...
    with tempfile.TemporaryDirectory(prefix=".bench-progression-", dir=root) as temp:
        ...
        command = [
            ...
            *(
                ["--disable-week2-shared-input-qkv"]
                if getattr(args, "disable_week2_shared_input_qkv", False)
                and variant.solution != "mlx"
                else []
            ),
            *(
                ["--disable-week2-io-aware-dense-attention"]
                if getattr(args, "disable_week2_io_aware_dense_attention", False)
                and variant.solution != "mlx"
                else []
            ),
            *(
                ["--disable-week2-shared-input-gate-up-swiglu"]
                if getattr(args, "disable_week2_shared_input_gate_up_swiglu", False)
                and variant.solution != "mlx"
                else []
            ),
            *variant.extra_args,
    ...
    if args.json_output:
        ...
        if args.matrix:
            ...
            configuration.update(
                matrix=True,
                prompt_lengths=prompt_lengths,
                disable_week2_shared_input_qkv=getattr(
                    args, "disable_week2_shared_input_qkv", False
                ),
                disable_week2_io_aware_dense_attention=getattr(
                    args, "disable_week2_io_aware_dense_attention", False
                ),
                disable_week2_shared_input_gate_up_swiglu=getattr(
                    args, "disable_week2_shared_input_gate_up_swiglu", False
                ),
            )
        payload = {
